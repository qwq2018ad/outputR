...
int main(int argc, char** argv)
{
    ...
    // check file open
    FILE* pointerPatternFile = NULL;  // 定義為 FILE* 型別

    if (fopen_s(&pointerPatternFile, argv[2], "rb") != 0) { // 傳遞 FILE** 並檢查返回值
        ...
        // 如果文件成功打開，印出成功訊息
        printf("Successfully opened the pattern file: \"%s\"\n", argv[2]);
    }
    ...
    pattern = (UC**)malloc(sizeof(UC*) * numberPattern);
    /* Request memory for the lengths of patterns */
    lengthPattern = (int*)malloc(sizeof(int) * numberPattern);
    ...
    for (i = 0; i < numberPattern; i++)
    {
        ...
        if (i == 0)
            ...
            lmin = lengthPattern[i];
        pattern[i] = (UC*)malloc(lengthPattern[i]);
    ...
    FILE* pointerTextFile = NULL; // 定義檔案指標

    // 正確使用 fopen_s
    if (fopen_s(&pointerTextFile, argv[1], "rb") != 0) { // 傳遞 FILE** 並檢查返回值
        ...
        for (int i = 0; i < numberPattern; i++) {
            free(pattern[i]);  // 釋放 pattern[i] 指向的記憶體
        }
        free(pattern);  // 釋放 pattern 指向的記憶體
        free(lengthPattern);  // 釋放 lengthPattern 指向的記憶體
        return 0;
    }
    ...
    bitText = (ULL*)calloc(totalNumberUllForText, sizeof(ULL));
    ...
    /* The distances between input string and 參考字串 */
    ...
    double allthetime_clock = (static_cast<double>(searchingEnd - preprocessingStart) / CLOCKS_PER_SEC) * 1000.0;

    /* Display the time for preprocessing phase, searching phase and total running */
    printf("%f %f %f\n", preprocessing_clock, searching_clock, allthetime_clock);
